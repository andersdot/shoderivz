...
def _leapfrog(x0,
  ...
  with ops.name_scope(name, 'leapfrog',
    ...
    def leap_to_tobs(x, v, leapfrog_state, num_times, step_size, k, i):
      ...
      with ops.name_scope('leapfrog_to_obs'):
        ...
        step_size, time, k, leapfrog_state, count = control_flow_ops.while_loop(
            time_fn, leapfrog_wrapper, (step_size, t_obs[i], k, leapfrog_state, 0.0),
            name='leapfrog_loop')
        dt_tiny = t_obs[i] - leapfrog_state.total_time1
        xt, vt, _, _ = _leap_frog_step(step_size, k, leapfrog_state)
        x = x.write(i, xt)
        v = v.write(i, vt)
    ...
    x = tensor_array_ops.TensorArray(
        x0.dtype, size=num_times).write(0, x0)
    v = tensor_array_ops.TensorArray(
        v0.dtype, size=num_times).write(0, v0)
...
def leapfrog(x0,
  ...
  with ops.name_scope(name, 'integrate', [x0, v0, k, t_obs]) as scope:
